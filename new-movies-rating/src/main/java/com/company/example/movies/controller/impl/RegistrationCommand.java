...
public class RegistrationCommand implements Command {
    ...
    public void execute(HttpServletRequest request, HttpServletResponse response) {
        ...
        if(!flag && !"".equalsIgnoreCase(login) && !"".equalsIgnoreCase(password)) {
            ...
            //response.sendRedirect("MainController?command=goToMainPage&message=регистрация успешна");//как параметр реквеста
            ...
            //response.sendRedirect("MainController?command=goToErrorPage&message=регистрация не удалась");//лучше редиректом(F5)
